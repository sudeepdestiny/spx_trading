...
class PaperTrader:
    """
    Paper/Live trading engine using IBKR data
    Updated with:
    - Safe hedge adjustment with margin checking
    - Combo order support with WhatIf margin preview
    - Iterative hedge adjustment (retry with closer hedges)
    """
        ...
        try:
            ...
            for strike in strikes:
                for right in ("C", "P"):
                    ...
                    contract.trading_class = option_trading_class
            ...
            logger.info(
                f"Finished option market data batches: "
    ...
    def place_combo_order(self, legs: List[Dict], net_price: float, action: str = "SELL", quantity: int = 1) -> int:
        """
        Place a combo (multi-leg) BAG order
        """
        from ibapi.contract import Contract, ComboLeg
        from ibapi.order import Order
        
        if not self.conn_mgr.is_connected:
            logger.error("IBKR not connected for combo order")
            return None
        
        client = self.conn_mgr.get_client()
        if client is None:
            logger.error("IBKR client unavailable for combo order")
            return None
        
        # Create combo contract (BAG = spread/combo)
        combo_contract = Contract()
        combo_contract.symbol = legs[0]["symbol"]
        combo_contract.secType = "BAG"
        combo_contract.exchange = legs[0].get("exchange", "CBOE")
        ...
        for leg in legs:
            ...
            if con_id is None:
                con_id = self.conn_mgr.resolve_option_conid(leg)
                if con_id is None:
        ...
        logger.info(f"Placing combo order: {len(combo_legs)} legs, net_price={net_price}, action={action}")
        client.placeOrder(order_id, combo_contract, order)
    ...
    def _adjust_hedge_with_margin_check(
        self,
        cycle_id: int,
        pos_data: dict,
        short_pos: dict,
        ...
        """
        Adjust hedge by moving it closer, with margin checking.
        Tries up to max_attempts with progressively closer hedges.
        ...
        logger.info(
            f"  Attempting hedge adjustment for cycle {cycle_id}: "
            f"old short={short_pos['strike']:.0f}, old hedge={hedge_pos['strike']:.0f}, type={option_type}"
        )
        
        for attempt in range(1, max_attempts + 1):
            reduced_hedge_dist = int(hedge_dist * (1.0 - 0.2 * attempt))  # Reduce by 20% each attempt
            if reduced_hedge_dist < 25:
                logger.warning(f"  Hedge distance too small ({reduced_hedge_dist}), stopping attempts")
                break
            
            logger.info(f"  Attempt {attempt}/{max_attempts}: trying hedge_dist={reduced_hedge_dist}")
            
            new_short = self._select_adjustment_short(option_chain, option_type, current_delta)
            if new_short is None:
                logger.warning(f"  No replacement {option_type} short found")
                continue
            
            hedge_row = self._find_hedge_row(
                option_chain,
                float(new_short["strike"]),
                option_type,
                reduced_hedge_dist,
                log_fallback=True,
            )
            if hedge_row is None:
                logger.warning(f"  No replacement {option_type} hedge found for dist={reduced_hedge_dist}")
                continue
            
            logger.info(
                f"  Proposed adjustment: {option_type} {short_pos['strike']:.0f}/{hedge_pos['strike']:.0f} → "
                f"{new_short['strike']:.0f}/{hedge_row['strike']:.0f} (hedge_dist={reduced_hedge_dist})"
            )
            
            # Line ~1015-1045, convert pandas Series to dict properly

    # Build 4-leg adjustment combo
            adjustment_legs = [
                # Close old short
                {
                    "symbol": short_pos["underlying"],
                    "secType": "OPT",
                    "exchange": "CBOE",
                    "currency": "USD",
                    "strike": short_pos["strike"],
                    "right": short_pos["right"],
                    "expiry": short_pos["expiry"],
                    "action": "BUY" if short_pos["action"] == "SELL" else "SELL",
                    "ratio": 1,
                },
                # Close old hedge
                {
                    "symbol": hedge_pos["underlying"],
                    "secType": "OPT",
                    "exchange": "CBOE",
                    "currency": "USD",
                    "strike": hedge_pos["strike"],
                    "right": hedge_pos["right"],
                    "expiry": hedge_pos["expiry"],
                    "action": "BUY" if hedge_pos["action"] == "SELL" else "SELL",
                    "ratio": 1,
                },
                # Open replacement short (FIX: use .get() for pandas Series)
                {
                    "symbol": new_short.get("underlying", "SPX"),
                    "secType": "OPT",
                    "exchange": "CBOE",
                    "currency": "USD",
                    "strike": float(new_short.get("strike", 0)),
                    "right": new_short.get("right", "C"),  # ← Use .get()
                    "expiry": str(new_short.get("expiry", "")),
                    "action": "SELL",
                    "ratio": 1,
                },
                # Open replacement hedge (FIX: use .get() for pandas Series)
                {
                    "symbol": hedge_row.get("underlying", "SPX"),
                    "secType": "OPT",
                    "exchange": "CBOE",
                    "currency": "USD",
                    "strike": float(hedge_row.get("strike", 0)),
                    "right": hedge_row.get("right", "C"),  # ← Use .get()
                    "expiry": str(hedge_row.get("expiry", "")),
                    "action": "BUY",
                    "ratio": 1,
                },
            ]

            # Calculate net price (simplified - should use actual bid/ask)
            net_price = 0.50  # Example - replace with actual calculation
            
            # Check margin with WhatIf
            margin_check = self.conn_mgr.check_combo_margin(
            ...
            logger.info(f"  Margin check result: {margin_check}")
            
            if not margin_check.get("can_trade"):
                logger.warning(
                    f"  Adjustment rejected by margin check: {margin_check.get('reason', 'unknown')} | "
                    f"warning={margin_check.get('warning_text', '')}"
                )
                # Try again with closer hedge
                continue
            
            # Margin OK - place real combo
            order_id = self.place_combo_order(
                legs=adjustment_legs,
                net_price=net_price,
                action="SELL",
            ...
            if order_id is None:
                logger.error("  Failed to submit adjustment combo order")
                return False
            
            logger.info(f"  Adjustment combo submitted: order_id={order_id}")
            ...
            replacement_short = self._position_from_row(new_short, "S", "SELL", option_type, quantity, cycle_id)
            replacement_hedge = self._position_from_row(hedge_row, "B", "BUY", option_type, quantity, cycle_id)
            
            if replacement_short and replacement_hedge:
                pos_data["positions"].extend([replacement_short, replacement_hedge])
                pos_data["adjustments"] = pos_data.get("adjustments", 0) + 1
                ...
                logger.info(f"  ✓ Adjustment successful: {short_pos['strike']:.0f}/{hedge_pos['strike']:.0f} → {replacement_short['strike']:.0f}/{replacement_hedge['strike']:.0f}")
                return True
            
            return False
        
        logger.error(f"  ✗ Hedge adjustment failed after {max_attempts} attempts")
        return False
    ...
    def monitor_and_adjust(self, option_chain):
        ...
        try:
            ...
            for cycle_id, pos_data in list(self.live_positions.items()):
                ...
                # Use the new safe adjustment method
                success = self._adjust_hedge_with_margin_check(
                    cycle_id=cycle_id,
                    pos_data=pos_data,
                    short_pos=short_pos,
                    hedge_pos=hedge_pos,
